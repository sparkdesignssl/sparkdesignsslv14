...
// ── THREE.JS — glossy rock pyramid flythrough (site-wide)
function initThree() {
  ...
  // Strong lighting for glossy rock look
  scene.add(new THREE.AmbientLight(0x0a0000, 2.0));
  const keyLight = new THREE.PointLight(0xff2200, 10, 40); keyLight.position.set(-5, 6, 6); scene.add(keyLight);
  const rimLight = new THREE.PointLight(0xffffff, 4, 28); rimLight.position.set(6, -4, 4); scene.add(rimLight);
  const fillLight = new THREE.PointLight(0xcc0000, 5, 30); fillLight.position.set(4, 5, -3); scene.add(fillLight);

  function rnd(min, max) { return min + Math.random() * (max - min); }

  // Build a proper square-base pyramid with displaced vertices for rocky look
  function makePyramidGeo(size) {
    ...
    // displace each vertex randomly for rocky surface
    for (let i = 0; i < verts.length; i += 3) {
      const isApex = (verts[i+1] > 0);
      const disp = isApex ? rnd(0.88, 1.12) : rnd(0.85, 1.15);
      verts[i]   *= disp;
      verts[i+2] *= disp;
      if (isApex) verts[i+1] *= rnd(0.9, 1.1);
    }
  ...
  function makeMat() {
    ...
    if (v === 0) return new THREE.MeshPhysicalMaterial({
      color: 0x111111, roughness: 0.68, metalness: 0.5,
      clearcoat: 1.0, clearcoatRoughness: 0.4,
      emissive: 0x330000, emissiveIntensity: 0.15,
    });
    if (v === 1) return new THREE.MeshPhysicalMaterial({
      color: 0x1e0000, roughness: 0.62, metalness: 0.58,
      clearcoat: 0.9, clearcoatRoughness: 0.32,
      emissive: 0x990000, emissiveIntensity: 0.3,
    });
    return new THREE.MeshPhysicalMaterial({
      color: 0x0a0505, roughness: 0.75, metalness: 0.44,
      clearcoat: 0.8, clearcoatRoughness: 0.52,
      emissive: 0x660000, emissiveIntensity: 0.2,
    });
  }

  function spawnPyramid(initScatter) {
    const size = rnd(0.25, 1.2);
    const mesh = new THREE.Mesh(makePyramidGeo(size), makeMat());
    // random initial rotation so they tumble at all angles
    ...
    mesh.position.z = initScatter ? rnd(-100, -4) : rnd(-105, -75);
    mesh.userData = {
      vz: rnd(0.005, 0.012),
      vx: rnd(-0.0015, 0.0015),
      vy: rnd(-0.001, 0.001),
  ...
  // No hero observer — runs site-wide always
  let tabVisible = true;
  ...
  function animate() {
    ...
    pyramids.forEach((mesh, i) => {
      ...
      // respawn when fully past camera
    ...
    keyLight.position.x = -5 + Math.sin(t * 0.28) * 1;
    keyLight.position.y = 6 + Math.cos(t * 0.2) * 0.8;
    rimLight.position.x = 6 + Math.sin(t * 0.16 + 1) * 0.9;
    keyLight.intensity = 9 + Math.sin(t * 0.45) * 1.2;
    fillLight.intensity = 4 + Math.sin(t * 0.38 + 1.5) * 1;
