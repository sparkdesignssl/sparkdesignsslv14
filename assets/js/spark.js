...
// ── THREE.JS — slow asteroid flythrough
function initThree() {
  ...
  const camera = new THREE.PerspectiveCamera(60, initW / initH, 0.1, 120);
  ...
  scene.add(new THREE.AmbientLight(0x0a0000, 2.5));
  const keyLight = new THREE.PointLight(0xff2200, 8, 30); keyLight.position.set(-4, 5, 5); scene.add(keyLight);
  const rimLight = new THREE.PointLight(0xffffff, 3, 20); rimLight.position.set(5, -3, 3); scene.add(rimLight);
  const fillLight = new THREE.PointLight(0xcc0000, 4, 25); fillLight.position.set(3, 4, -2); scene.add(fillLight);

  function rnd(min, max) { return min + Math.random() * (max - min); }

  function makeRockyGeo(baseSize) {
    const geo = new THREE.TetrahedronGeometry(baseSize, 1);
    const pos = geo.attributes.position;
    for (let i = 0; i < pos.count; i++) {
      const ox = pos.getX(i), oy = pos.getY(i), oz = pos.getZ(i);
      const len = Math.sqrt(ox*ox + oy*oy + oz*oz);
      const disp = rnd(0.82, 1.18);
      pos.setXYZ(i, ox/len * len * disp, oy/len * len * disp, oz/len * len * disp);
    }
  ...
  function makeMat() {
    ...
    if (v === 0) return new THREE.MeshPhysicalMaterial({color:0x0c0c0c, roughness:0.72, metalness:0.55, clearcoat:0.9, clearcoatRoughness:0.5, emissive:0x330000, emissiveIntensity:0.12});
    if (v === 1) return new THREE.MeshPhysicalMaterial({color:0x1c0000, roughness:0.65, metalness:0.6, clearcoat:1.0, clearcoatRoughness:0.38, emissive:0x880000, emissiveIntensity:0.28});
    return new THREE.MeshPhysicalMaterial({color:0x080404, roughness:0.78, metalness:0.48, clearcoat:0.7, clearcoatRoughness:0.6, emissive:0x550000, emissiveIntensity:0.18});
  }

  function spawnAsteroid(initScatter) {
    const size = rnd(0.3, 1.4);
    const mesh = new THREE.Mesh(makeRockyGeo(size), makeMat());
    const spread = 14;
    mesh.position.x = rnd(-spread, spread);
    mesh.position.y = rnd(-spread * 0.6, spread * 0.6);
    mesh.position.z = initScatter ? rnd(-90, -5) : rnd(-100, -70);
    mesh.userData = {
      vz: rnd(0.006, 0.014),
      vx: rnd(-0.002, 0.002),
      vy: rnd(-0.0015, 0.0015),
      rx: rnd(0.0003, 0.0015) * (Math.random() < 0.5 ? 1 : -1),
      ry: rnd(0.0005, 0.002) * (Math.random() < 0.5 ? 1 : -1),
      rz: rnd(0.0002, 0.001) * (Math.random() < 0.5 ? 1 : -1),
      size,
    };
    scene.add(mesh);
    return mesh;
  ...
  const COUNT = 14;
  const asteroids = [];
  for (let i = 0; i < COUNT; i++) asteroids.push(spawnAsteroid(true));
  ...
  function animate() {
    ...
    asteroids.forEach((mesh, i) => {
      ...
      if (mesh.position.z > d.size + 2) {
        scene.remove(mesh);
        mesh.geometry.dispose();
        mesh.material.dispose();
        asteroids[i] = spawnAsteroid(false);
      }
    });

    keyLight.position.x = -4 + Math.sin(t * 0.3) * 0.8;
    keyLight.position.y = 5 + Math.cos(t * 0.22) * 0.6;
    rimLight.position.x = 5 + Math.sin(t * 0.18 + 1) * 0.7;
    keyLight.intensity = 7 + Math.sin(t * 0.5) * 1;
    fillLight.intensity = 3.5 + Math.sin(t * 0.4 + 1.2) * 0.8;
