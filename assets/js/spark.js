...
// ── THREE.JS — pause when hero off screen + tab hidden
function initThree() {
  ...
  scene.add(new THREE.PointLight(0xffffff,1.4,8)).position.set(0,3,3);

  // MeshStandardMaterial — no clearcoat, much faster
  const blackMat = new THREE.MeshStandardMaterial({color:0x030303, roughness:0.18, metalness:0.82, emissive:0x220000, emissiveIntensity:0.08});
  const redMat = new THREE.MeshStandardMaterial({color:0x160000, roughness:0.16, metalness:0.88, emissive:0x660000, emissiveIntensity:0.22});
  const geo = new THREE.TetrahedronGeometry(0.72,0);

  const leftGroup = new THREE.Group(); leftGroup.position.set(-2.8,0,-1);
  ...
  const rightGroup = new THREE.Group(); rightGroup.position.set(2.8,0,-1);
  ...
  let leftBaseX = -2.8, rightBaseX = 2.8;
  ...
  function layoutPyramids() {
    ...
    // compute how far from centre we can place groups and still be fully in frame
    // frustum half-width at z=-1 (groups z offset)
    const dist = cz - (-1); // camera z minus group z
    const fovRad = (60 * Math.PI) / 180;
    const frustumHalfH = Math.tan(fovRad / 2) * dist;
    const aspect = (canvas.offsetWidth || w) / (canvas.offsetHeight || window.innerHeight);
    const frustumHalfW = frustumHalfH * aspect;

    // leave margin so meshes (radius ~0.72 + offset ~0.22) don't clip
    const margin = mob ? 1.05 : 1.2;
    const xPos = Math.max(1.0, frustumHalfW - margin);

    if (mob) {
      leftBaseX = -xPos; rightBaseX = xPos;
      leftGroup.position.set(-xPos, 1.4, -1);
      rightGroup.position.set(xPos, -1.4, -1);
      leftGroup.scale.setScalar(0.55);
      rightGroup.scale.setScalar(0.55);
    } else {
      leftBaseX = -xPos; rightBaseX = xPos;
      leftGroup.position.set(-xPos, 0, -1);
      rightGroup.position.set(xPos, 0, -1);
  ...
  const heroObs = new IntersectionObserver(entries => { heroVisible = entries[0].isIntersecting; if(heroVisible) animate(); }, {threshold:0});
  heroObs.observe(document.getElementById('hero'));

  // Pause when tab hidden
  let tabVisible = true;
  document.addEventListener('visibilitychange', () => { tabVisible = !document.hidden; if(tabVisible && heroVisible) animate(); });
  ...
  function animate() {
    ...
    // slow organic drift — layered sines with different periods per group
    const lx = Math.sin(t*0.11)*0.55 + Math.sin(t*0.07+1.2)*0.35 + Math.sin(t*0.19+2.4)*0.18;
    const ly = Math.sin(t*0.13+0.5)*0.45 + Math.sin(t*0.08+2.1)*0.30 + Math.sin(t*0.22+0.8)*0.15;
    const rx = Math.sin(t*0.09+3.1)*0.55 + Math.sin(t*0.06+1.7)*0.35 + Math.sin(t*0.17+0.3)*0.18;
    const ry = Math.sin(t*0.12+1.8)*0.45 + Math.sin(t*0.10+3.4)*0.30 + Math.sin(t*0.21+1.5)*0.15;

    leftGroup.position.x += (leftBaseX + lx - leftGroup.position.x) * 0.012;
    leftGroup.position.y += (ly - leftGroup.position.y) * 0.012;
    rightGroup.position.x += (rightBaseX + rx - rightGroup.position.x) * 0.012;
    rightGroup.position.y += (ry - rightGroup.position.y) * 0.012;
