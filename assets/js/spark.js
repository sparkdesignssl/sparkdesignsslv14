...
function boot() {
  ...
  initLightning();
  animHero();
  gsap.to('.pyramid', { opacity: 0.72, duration: 1.2, stagger: 0.15, delay: 0.2 });
...
function initLetterGlow() {
  document.querySelectorAll('.section-title-glow').forEach(el => {
    ...
    const trigger = el.closest('.section-label') || el;
    const obs = new IntersectionObserver(entries => {
      if (entries[0].isIntersecting) {
        const letters = el.querySelectorAll('.glow-letter');
        letters.forEach(s => { s.classList.remove('lit'); void s.offsetWidth; });
        letters.forEach((s, i) => {
          setTimeout(() => s.classList.add('lit'), i * 70);
        });
      }
    }, { threshold: 0.15 });
    obs.observe(trigger);
...
albumStage.addEventListener('mousedown', e => { dragStart=e.clientY; dragLast=e.clientY; });
...
function initThree() {
  ...
  function layoutPyramids() {
    ...
    camera.position.z = mob ? 6.2 : 5;
    leftGroup.position.x = mob ? -1.92 : -3.8;
    rightGroup.position.x = mob ? 1.92 : 3.8;
    leftGroup.scale.setScalar(mob?0.78:1);
    rightGroup.scale.setScalar(mob?0.78:1);
  }
  layoutPyramids();
  window.addEventListener('resize', () => { clearTimeout(threeResizeTimer); threeResizeTimer=setTimeout(layoutPyramids,150); });
