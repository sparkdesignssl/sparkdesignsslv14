...
// ── THREE.JS — asteroid-style drifting pyramids
function initThree() {
  ...
  const keyLight = new THREE.PointLight(0xff1a1a, 4.5, 18); keyLight.position.set(-3, 3, 4); scene.add(keyLight);
  const fillLight = new THREE.PointLight(0xcc0000, 3.2, 14); fillLight.position.set(4, -2, 3); scene.add(fillLight);
  scene.add(new THREE.PointLight(0xffffff, 1.4, 10)).position.set(0, 4, 3);

  const mats = [
    new THREE.MeshPhysicalMaterial({color:0x080808, roughness:0.55, metalness:0.7, clearcoat:0.6, clearcoatRoughness:0.35, emissive:0x220000, emissiveIntensity:0.1}),
    new THREE.MeshPhysicalMaterial({color:0x1a0000, roughness:0.48, metalness:0.75, clearcoat:0.8, clearcoatRoughness:0.28, emissive:0x660000, emissiveIntensity:0.24}),
    new THREE.MeshPhysicalMaterial({color:0x0d0000, roughness:0.62, metalness:0.65, clearcoat:0.5, clearcoatRoughness:0.42, emissive:0x440000, emissiveIntensity:0.15}),
  ];

  // compute frustum bounds at z=0 plane
  function getFrustum() {
  ...
  // spawn an asteroid pyramid
  function spawnAsteroid(scatterInit) {
    const size = rnd(0.28, 1.1);
    const geo = new THREE.TetrahedronGeometry(size, 0);
    const mat = mats[Math.floor(Math.random() * mats.length)];
    const mesh = new THREE.Mesh(geo, mat);

    const { halfW, halfH } = getFrustum();
    const pad = size + 0.2;

    let x, y;
    if (scatterInit) {
      // scatter randomly across whole screen at start
      x = rnd(-halfW + pad, halfW - pad);
      y = rnd(-halfH + pad, halfH - pad);
    } else {
      // spawn from a random edge
      const edge = Math.floor(Math.random() * 4);
      if (edge === 0) { x = rnd(-halfW - pad, halfW + pad); y = -halfH - pad; }       // bottom
      else if (edge === 1) { x = rnd(-halfW - pad, halfW + pad); y = halfH + pad; }   // top
      else if (edge === 2) { x = -halfW - pad; y = rnd(-halfH - pad, halfH + pad); }  // left
      else { x = halfW + pad; y = rnd(-halfH - pad, halfH + pad); }                   // right
    }

    mesh.position.set(x, y, rnd(-2, 0.5));

    // slow random velocity — drift speed varies with size (smaller = faster)
    const speed = rnd(0.003, 0.008) / Math.sqrt(size);
    const angle = rnd(0, Math.PI * 2);
    mesh.userData = {
      vx: Math.cos(angle) * speed,
      vy: Math.sin(angle) * speed,
      rx: rnd(0.001, 0.004) * (Math.random() < 0.5 ? 1 : -1),
      ry: rnd(0.002, 0.005) * (Math.random() < 0.5 ? 1 : -1),
      rz: rnd(0.001, 0.003) * (Math.random() < 0.5 ? 1 : -1),
      size,
    };
  ...
  function animate() {
    ...
    asteroids.forEach((mesh, i) => {
      ...
      // respawn only when fully off screen — pad by actual mesh size
      const killPad = d.size + 0.5;
      if (
        mesh.position.x > halfW + killPad ||
        mesh.position.x < -halfW - killPad ||
        mesh.position.y > halfH + killPad ||
        mesh.position.y < -halfH - killPad
      ) {
        scene.remove(mesh);
    ...
    keyLight.position.x = -3 + Math.sin(t * 0.34) * 0.8;
